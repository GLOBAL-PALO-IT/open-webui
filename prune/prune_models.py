...
class PrunePreviewResult(BaseModel):
    ...
    orphaned_uploads: list[str] = []
    orphaned_vector_collections: list[dict[str, str]] = []  # Enriched collection info with type, id, and name
    embedded_file_records: dict[str, list[str]] = {}  # File names with vector embeddings, grouped by knowledge base
    embedded_upload_files: dict[str, list[str]] = {}  # Upload file names with vector embeddings, grouped by knowledge base
    non_embedded_files: dict[str, list[str]] = {}  # File names without vector embeddings, grouped by knowledge base
    ...
    def total_items(self) -> int:
        ...
        return (
            ...
            len(self.orphaned_uploads) +
            len(self.orphaned_vector_collections) +
            sum(len(files) for files in self.embedded_file_records.values()) +
            sum(len(files) for files in self.embedded_upload_files.values()) +
    ...
    def get_summary_dict(self) -> dict:
        ...
        return {
            ...
            "Embedded Files (by Knowledge Base)": {
                "File records with embeddings": self.embedded_file_records,
                "Upload files with embeddings": self.embedded_upload_files,
            },
            "Workspace": {
