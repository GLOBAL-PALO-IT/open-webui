...
class ChromaDatabaseCleaner(VectorDatabaseCleaner):
    ...
    def get_orphaned_collection_names(
        ...
        """Get list of orphaned ChromaDB collection names."""
        ...
        try:
            for collection_dir in self.vector_dir.iterdir():
                ...
                if (
                    ...
                    orphaned.append(collection_name or f"<unknown: {dir_uuid[:8]}...>")
        except Exception as e:
            log.debug(f"Error getting orphaned ChromaDB collections: {e}")
    ...
    def count_orphaned_collections(
        ...
        return len(self.get_orphaned_collection_names(active_file_ids, active_kb_ids, active_user_ids))
...
class PGVectorDatabaseCleaner(VectorDatabaseCleaner):
    ...
    def get_orphaned_collection_names(
        ...
        """Get list of orphaned PGVector collection names."""
        if not self.session:
            return []
        try:
            ...
            return sorted(list(orphaned_collections))
        except Exception as e:
            if self.session:
                self.session.rollback()
            log.error(f"Error getting orphaned PGVector collection names: {e}")
            return []
...
class MilvusDatabaseCleaner(VectorDatabaseCleaner):
    ...
    def get_orphaned_collection_names(
        ...
        """Get list of orphaned Milvus collection names."""
        try:
            expected_collections = self._build_expected_collections(
                active_file_ids, active_kb_ids
            )
            ...
            # Find collections with our prefix that are not expected
            orphaned = []
            for collection_name in all_collections:
                if collection_name.startswith(f"{self.collection_prefix}_"):
                    ...
                    if original_name not in expected_collections:
                        orphaned.append(original_name)

            return sorted(orphaned)

        except Exception as e:
            log.error(f"Error getting orphaned Milvus collection names: {e}")
            return []
...
class MilvusMultitenancyDatabaseCleaner(VectorDatabaseCleaner):
    ...
    def get_orphaned_collection_names(
        ...
        """Get list of orphaned resource_ids across shared Milvus collections."""
        try:
            ...
            for shared_collection_name in self.shared_collections:
                ...
                try:
                    ...
                    # Find orphaned ones (limit to first 100 to avoid huge lists)
                    for resource_id in sorted(all_resource_ids):
                        if resource_id not in expected_resource_ids:
                            orphaned.append(f"{shared_collection_name.split('_')[-1]}:{resource_id[:30]}")
                            if len(orphaned) >= 100:
                                break

                except Exception as e:
                    log.error(f"Error scanning Milvus collection {shared_collection_name}: {e}")

            return orphaned

        except Exception as e:
            log.error(f"Error getting orphaned Milvus multitenancy resource IDs: {e}")
            return []
...
class QdrantDatabaseCleaner(VectorDatabaseCleaner):
    ...
    def get_orphaned_collection_names(
        ...
        """Get list of orphaned Qdrant collection names."""
        try:
            expected_collections = self._build_expected_collections(
                active_file_ids, active_kb_ids
            )
            ...
            log.error(f"Error getting orphaned Qdrant collection names: {e}")
            return []
...
class QdrantMultitenancyDatabaseCleaner(VectorDatabaseCleaner):
    ...
    def get_orphaned_collection_names(
        ...
        """Get list of orphaned tenant_ids across shared Qdrant collections."""
        try:
            ...
            for collection_name in self.shared_collections:
                ...
                try:
                    ...
                    # Find orphaned tenant_ids (limit to first 100)
                    for tenant_id in sorted(all_tenant_ids):
                        if tenant_id not in expected_tenant_ids:
                            orphaned.append(f"{collection_name.split('_')[-1]  }:{tenant_id[:30]}")
                            if len(orphaned) >= 100:
                                break
            ...
            log.error(f"Error getting orphaned Qdrant multitenancy tenant IDs: {e}")
            return []
...
class NoOpVectorDatabaseCleaner(VectorDatabaseCleaner):
    ...
    def get_orphaned_collection_names(
        ...
        """No orphaned collections to list for unsupported databases."""
        return []
